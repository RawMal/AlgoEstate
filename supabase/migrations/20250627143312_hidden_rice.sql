/*
  # Complete Real Estate Tokenization Database Schema

  1. New Tables
    - `properties` - Store tokenized real estate properties
    - `users` - User accounts with wallet addresses and KYC status
    - `token_ownership` - Track property token ownership
  ...
  3. Functions
    - `update_available_tokens()` - Automatically update token availability
    - `get_user_portfolio()` - Get user's property investments
    - `get_user_portfolio_detailed()` - Extended portfolio information
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_properties_asa_id') THEN
    CREATE INDEX idx_properties_asa_id ON properties(asa_id);
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_token_ownership_property_id') THEN
    CREATE INDEX idx_token_ownership_property_id ON token_ownership(property_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_token_ownership_user_id') THEN
    CREATE INDEX idx_token_ownership_user_id ON token_ownership(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_token_ownership_wallet_address') THEN
    CREATE INDEX idx_token_ownership_wallet_address ON token_ownership(wallet_address);
  END IF;
END $$;
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Users can view all token ownership records" ON token_ownership;
  DROP POLICY IF EXISTS "Users can insert their own token ownership" ON token_ownership;
  DROP POLICY IF EXISTS "Users can update their own token ownership" ON token_ownership;
